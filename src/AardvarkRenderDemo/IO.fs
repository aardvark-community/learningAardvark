...
module Loader =
    ...
    type Scene = 
        ...
        member x.SubstituteMaterial (mapping : IMaterial -> Option<IMaterial>) =
    ...
    module SgSems =
        ...
        type Node with      
            ...
            member x.ModelTrafo(scope : Ag.Scope)  : aval<Trafo3d> = x?ModelTrafo(scope)
        ...
        type SceneSem() =
            ...
            member x.GlobalBoundingBox(s : Scene, scope : Ag.Scope) =
                s.ModelTrafo(scope) |> AVal.map (fun t -> s.bounds.Transformed t)
